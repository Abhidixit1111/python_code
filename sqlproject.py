...
conn= mysql.connect(host='localhost', user='root', password="abhisheksql1111",
                            database="ssm")
cur=conn.cursor()
#cur.execute("CREATE TABLE student(id integer(4),name varchar(20),phone varchar(10))")
...
def delete():
    ...
    if(e_id.get()==""):
        ...
        conn= mysql.connect(host='localhost', user='root', password="abhisheksql1111",
                            database="ssm")
        cur=conn.cursor()
        cur.execute("DELETE FROM student WHERE id='"+ e_id.get() +"'")
        conn.commit()
...
def update():
    ...
    if(id=="" or name=="" or phone==""):
        ...
        conn= mysql.connect(host='localhost', user='root', password="abhisheksql1111",
                            ...
                            database="ssm")
        cur=conn.cursor()
        cur.execute("UPDATE student SET name='"+ name +"', phone='"+ phone +"' WHERE id='"+ id +"'")
        conn.commit()
...
def get():
    ...
    if(e_id.get()==""):
        ...
        conn= mysql.connect(host='localhost', user='root', password="abhisheksql1111",
                            database="ssm")
        cur=conn.cursor()
        cur.execute("SELECT * FROM student WHERE id='"+ e_id.get() +"'")
        rows=cur.fetchall()
...
insert= Button(root, text="Insert", font=("italic", 10), bg="lightblue", command=insert)
insert.place(x=20, y=140)

delete= Button(root, text="Delete", font=("italic", 10), bg="lightblue", command=delete)
delete.place(x=65, y=140)

update= Button(root, text="Update", font=("italic", 10), bg="lightblue", command=update)
update.place(x=115, y=140)

get= Button(root, text="Get", font=("italic", 10), bg="lightblue", command=get)
